...
function buildSessionCookie(token, maxAge = 60 * 60 * 24 * 30) {
...
export async function onRequest(context) {
  ...
  if (request.method === "GET" && action === "login") {
    ...
    if (!config || !config.clientId || !config.clientSecret) {
      return json(500, { success: false, errors: [{ message: `Missing OAuth config for ${provider}` }] });
    }
  ...
  if (request.method === "GET" && action === "callback") {
    const config = providerConfig(provider, env, url.origin);
    if (!config) return json(400, { success: false, errors: [{ message: "Unknown provider" }] });

    const code = url.searchParams.get("code");
    const state = url.searchParams.get("state");
    if (!code || !state) {
      return json(400, { success: false, errors: [{ message: "Missing code/state" }] });
    }

    const stateEntry = await adapter.get(`auth:state:${state}`);
    if (!stateEntry) return json(400, { success: false, errors: [{ message: "Invalid auth state" }] });
    await adapter.del(`auth:state:${state}`);

    const tokenPayload = await exchangeToken(provider, config, code);
    const accessToken = tokenPayload.access_token;
    if (!accessToken) return json(400, { success: false, errors: [{ message: "Missing access token" }] });
